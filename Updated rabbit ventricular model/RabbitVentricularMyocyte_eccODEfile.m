function ydot = RabbitVentricularMyocyte_eccODEfile(t,y,p)
...
GNa =  par_SA(1)*16.0;        % [mS/uF]
GNaL = par_SA(2)*2*0.0065*2.5;    % [mS/uF] 
GNaB = par_SA(3)*0.297e-3;    % [mS/uF] 

IbarNaK = par_SA(4)*1.90719;     % [uA/uF]
KmNaip = 11;         % [mM]
KmKo = 1.5;         % [mM]
Q10NaK = 1.63;  
Q10KmNai = 1.39;
...
GtoFast = par_SA(5)*0.02; % 25% [mS/uF] 
GtoSlow = par_SA(6)*0.06; % 75% [mS/uF] 
%     GtoTot = 0.08;       %total 0.08 in Ctrl, decrease 1/3 in HF
%     GtoSlow = (0.65*GtoTot)*par_SA(6); %65% slow component in ctrl and HF
%     GtoFast = (0.35*GtoTot)*par_SA(5);

pNaK = 0.01833;
gks_factor_SA = 1.4*par_SA(7);
gkr = 1.4*par_SA(8)*0.03*sqrt(Ko/5.4); % 1.21
gkp = par_SA(12)*0.001;
gki = par_SA(13)*0.9*sqrt(Ko/5.4); % 0.778
%% Cl current parameters

gCFTR = par_SA(14)*4.9e-3; % [A/F] - Max value as in Shannon et al. (2005) with max phosphorylation
GClCa = par_SA(15)*0.109625;   % [mS/uF]
GClB = 0.5*par_SA(16)*9e-3; %%% % [mS/uF]
KdClCa = 100e-3;    % [mM]
%% Ca transport parameters

K_Ica = par_SA(17)*(0.92); %%%
pCa = K_Ica*5.4e-4;       % [cm/sec]
pNa = K_Ica*1.5e-8;       % [cm/sec]
pK = K_Ica*2.7e-7;        % [cm/sec]
KmCa = 0.6e-3; % [mM]
Q10CaL = 1.8;       

GCaB = par_SA(18)*2.513e-4;    % [uA/uF] 

IbarNCX = par_SA(19)*9.0;      % [uA/uF]
KmCai = 3.59e-3;    % [mM]
KmCao = 1.3;        % [mM]
KmNai = 12.29;      % [mM]
KmNao = 87.5;       % [mM]
...
% SR flux parameters
Q10SRCaP = 2.6;          % [none]
Vmax_SRCaP = par_SA(21)*2.86e-4;  % [mM/msec] (mmol/L cytosol/msec)
Kmf = 0.246e-3;          % [mM]
...
fracCFTRpo = 0.2324; % Derived quantity - (CFTRp(baseline)/CFTRtot)
...
% CaMKII-dependent INa phosphoregulation
...
if IKs_flag == 0 % OLD IKS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    I_ks_sl = Fsl*gks_factor_SA*gks_sl*y(13)^2*(y(39)-eks);
else % NEW IKS - 2016 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%favail = 1+0.5*(LCCb_PKAp - fracLCCbpo)/(1 - fracLCCbpo); % 1.5* with max phosphorylation
    favail = 1+1.25*(LCCb_PKAp - fracLCCbpo)/(1 - fracLCCbpo); % PROVA 2.25* with max phosphorylation
...
Icftr = fact_pka_cftr*gCFTR*(y(39) - ecl);
