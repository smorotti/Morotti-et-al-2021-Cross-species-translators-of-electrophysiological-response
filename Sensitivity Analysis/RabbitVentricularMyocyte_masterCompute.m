%% Main file: RabbitVentricularMyocyte_masterCompute
...
% NEW - Myofilament unplugged
%save yf_rvm_1Hz_NEW yfinal
%save yf_rvm_1Hz_120s_ISO_NEW yfinal
%% Rename outputs
...
IKr = Ikr_store(1:tStep); IKs = IKs_store(1:tStep);
IKp = IKp_store(1:tStep); ICFTR = ICFTR_store(1:tStep);
...
legend('IKs','IKr')
xlabel('Time (ms)')
subplot(4,2,8); hold on, plot(t,ICFTR_PKAp*100); ylabel('CFTR/ClCa (%)');
set(gca,'box','off','tickdir','out','fontsize',12)
xlabel('Time (ms)')
