function ydot = HumanVentricularMyocyte_eccODEfile(t,y,p)
...
LCCa_PKAp = p(10);
LCCb_PKAp = p(11);
PLB_PKAn = p(12); % non-phosphorylated PLB targets
RyR_PKAp = p(13);
TnI_PKAp = p(14);
IKs_PKAp = p(15);
PLM_PKAp = p(16);
...
GNa = 0.9* par_SA(1)*16;        % [mS/uF]  %%%
GNaL = par_SA(2)*2*0.0065*2.5;        % [mS/uF]
GNaB = 0.5*par_SA(3)*0.597e-3;    % [mS/uF] 0.897e-3 %%%

IbarNaK = par_SA(4)*1.8;%1.90719;     % [uA/uF]
KmNaip = 11;         % [mM]
KmKo = 1.5;         % [mM]
Q10NaK = 1.63;  
Q10KmNai = 1.39;
...
if epi == 1
    GtoFast = par_SA(5)*0.13*0.88; % epi 88% 0.1144 [mS/uF]
    GtoSlow = par_SA(6)*0.13*0.12; % epi 12% 0.0156 [mS/uF]
else
    GtoFast = par_SA(5)*0.13*0.3*0.036; % endo 4% 0.0014 [mS/uF]
    GtoSlow = par_SA(6)*0.13*0.3*0.964; % endo 96% 0.0376 [mS/uF]
end
pNaK = 0.01833; 
gks_factor_SA = 2.5*par_SA(7);  %%%
gkr = par_SA(8)*(3+0.5)*0.035*sqrt(Ko/5.4); % added 3*gkr from Pei-Chi
gkp = par_SA(12)*2*0.001;
gki = par_SA(13)*0.35*sqrt(Ko/5.4);
%% Cl current parameters

GClCa = 2*par_SA(15)*(0.5*0.109625);   % [mS/uF]  %%%
GClB = 0.5*par_SA(16)*9e-3;        % [mS/uF]  %%%
KdClCa = 100e-3;    % [mM]
%% Ca transport parameters

K_Ica = par_SA(17)*(0.47); % added 0.47*GCaL from Pei-Chi %%%
pCa = K_Ica*5.4e-4;       % [cm/sec] % Markov model
pNa = K_Ica*1.5e-8;       % [cm/sec]
pK = K_Ica*2.7e-7;        % [cm/sec]
Q10CaL = 1.8;       

GCaB = 0.5*par_SA(18)*5.513e-4;    % [uA/uF] %%%

IbarNCX = par_SA(19)*4.5;      % [uA/uF]5.5 before - 9 in rabbit
KmCai = 3.59e-3;    % [mM]
KmCao = 1.3;        % [mM]
KmNai = 12.29;      % [mM]
KmNao = 87.5;       % [mM]
...
IbarSLCaP = par_SA(20)*0.0673; % IbarSLCaP FEI changed [uA/uF](2.2 umol/L cytosol/sec) jeff 0.093 [uA/uF]
KmPCa = 0.5e-3;     % [mM] 
Q10SLCaP = 2.35;    % [none]

% SR flux parameters
Q10SRCaP = 2.6;          % [none]
Vmax_SRCaP = par_SA(21)*5.3114e-3;  % [mM/msec] (286 umol/L cytosol/sec)
...
GbarK = (1+0.5*CKIIflag)*((14/11)/(1+exp(-(Nav_CKp-0.12)/0.1))); %(current) includes dynamnic CMKII phosphorylation in control and HF
...
I_NaL_junc = Fjunc*GbarNal*y(1)^3*y(41)*(y(39)-ena_junc);
I_NaL_sl = Fsl*GbarNal*y(1)^3*y(41)*(y(39)-ena_sl);
I_NaL = I_NaL_junc+I_NaL_sl;
...
if IKs_flag == 0 % OLD IKS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    xsss = 1 / (1+exp(-(y(39) + 3.8 - Xs05)/14.25)); % fitting Fra
    ...
    I_ks_junc = Fjunc*gks_factor_SA*gks_junc*y(13)^2*(y(39)-eks);
    I_ks_sl = Fsl*gks_factor_SA*gks_sl*y(4)^2*(y(39)-eks);    
end
