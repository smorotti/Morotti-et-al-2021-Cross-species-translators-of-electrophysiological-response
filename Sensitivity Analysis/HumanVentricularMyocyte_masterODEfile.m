function dydt = HumanVentricularMyocyte_masterODEfile(t,y,p)
...
pBAR = [Ligtot,PP1_PLBtot,PLBtotBA,PLMtotBA,LCCtotBA,...
    ...
    IClCatotBA,ItototBA,IK1totBA,INatotBA];
...
pECC = [prot_index,cycleLength,prot_input_par,CKIIOE,Ca_clamp,Na_clamp,...
    ...
    IKs_PKAp,PLM_PKAp,Myo_PKAp,IKr_PKAp,IClCa_PKAp,...
...
if flag_ECC == 0
    ...
    dydt_ecc = HumanVentricularMyocyte_eccODEfile(t,y_ecc,pECC);
end
...
if flag_cam == 0
    ...
    dydt_camDyad = HumanVentricularMyocyte_camODEfile(t,y_camDyad,pCaMDyad);
    dydt_camSL = HumanVentricularMyocyte_camODEfile(t,y_camSL,pCaMSL);
    dydt_camCyt = HumanVentricularMyocyte_camODEfile(t,y_camCyt,pCaMCyt);
end

if flag_CaMKII == 0
    dydt_CaMKIIDyad = zeros(1,length(y_CaMKII))';
else
    dydt_CaMKIIDyad = HumanVentricularMyocyte_camkiiODEfile(t,y_CaMKII,pCaMKII);
end

if flag_BAR == 0
    dydt_BAR = zeros(1,length(y_BAR))';
else
    dydt_BAR = HumanVentricularMyocyte_barODEfile(t,y_BAR,pBAR);
end
