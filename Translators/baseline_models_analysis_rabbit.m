...
% Protocol
prot_index = 1; % 0 no stim, 1 pace, 2 v-step, 3 AP-clamp
...
title('Baseline Rabbit model (control, 1 Hz)')
...
outputs_rabbit_baseline = outputs;
...
parfor ii = 1:length(block_index)
    ...
    tspan = [0 300e3]; % [ms]
    [t,y] = ode15s(@RabbitVentricularMyocyte_masterODEfile,tspan,y0n_baseline,options,p);
...
for ii = 1:length(block_index)
    ...
    tspan = [0 3e3]; % [ms]
    [t,y] = ode15s(@RabbitVentricularMyocyte_masterODEfile,tspan,yfinal_matrix(ii,:),options,p);
...
index_rabbit_block = block_index;
outputs_rabbit_block = output_matrix;
%% ISO

load yf_rvm_1Hz_120s_ISO_NEW
y0n = yfinal;
...
figure,set(gcf,'color','w')
subplot(4,1,1); hold on, plot(t,y(:,39)); ylabel('Em (mV)');
title('Baseline Rabbit model (ISO, 1 Hz)')
set(gca,'box','off','tickdir','out','fontsize',12)
subplot(4,1,2); hold on, plot(t,y(:,38)*1e6); ylabel('[Ca]i (nM)');
set(gca,'box','off','tickdir','out','fontsize',12)
subplot(4,1,3); hold on, plot(t,y(:,31)); ylabel('[Ca]SR (mM)');
...
outputs_rabbit_iso = outputs;
%% Saving

%save outputs_current_block_rabbit index_rabbit_block outputs_rabbit_baseline outputs_rabbit_block outputs_rabbit_iso
