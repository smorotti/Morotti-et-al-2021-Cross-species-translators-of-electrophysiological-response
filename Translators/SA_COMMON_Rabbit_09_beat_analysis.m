...
load ICs_matrix_1500_0p1_300s_rabbit_COMMON_3Hz
...
prot_freq = 3; % [Hz] CHANGE DEPENDING ON FREQUENCY
...
%     subplot(2,1,1); hold on, plot(t,y(:,39)); ylabel('Em (mV)');
%     set(gca,'box','off','tickdir','out','fontsize',12)
%     subplot(2,1,2); hold on, plot(t,y(:,38)*1e6); ylabel('[Ca]i (nM)');
%     set(gca,'box','off','tickdir','out','fontsize',12)
%     xlabel('Time (ms)')
...
%     xlabel('Time (ms)')
    ...
    X = sprintf('Done %d on %d',ii,N_trials); disp(X)
    
%     if rem(ii,10) == 0, figure(101), close, end
end
...
%% Saving
%save outputs_matrix_1500_0p1_300s_rabbit_COMMON_3Hz all_outputs output_names output_units
