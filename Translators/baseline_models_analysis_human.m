...
title('Baseline Human model (control, 1 Hz)')
...
outputs_human_baseline = outputs;
...
parfor ii = 1:length(block_index)
    ...
    tspan = [0 300e3]; % [ms]
    [t,y] = ode15s(@HumanVentricularMyocyte_masterODEfile,tspan,y0n_baseline,options,p);
...
for ii = 1:length(block_index)
    ...
    tspan = [0 3e3]; % [ms]
    [t,y] = ode15s(@HumanVentricularMyocyte_masterODEfile,tspan,yfinal_matrix(ii,:),options,p);
...
index_human_block = block_index;
outputs_human_block = output_matrix;
%% ISO
...
figure,set(gcf,'color','w')
subplot(4,1,1); hold on, plot(t,y(:,39)); ylabel('Em (mV)');
title('Baseline Human model (ISO, 1 Hz)')
set(gca,'box','off','tickdir','out','fontsize',12)
subplot(4,1,2); hold on, plot(t,y(:,38)*1e6); ylabel('[Ca]i (nM)');
set(gca,'box','off','tickdir','out','fontsize',12)
subplot(4,1,3); hold on, plot(t,y(:,31)); ylabel('[Ca]SR (mM)');
...
outputs_human_iso = outputs;
%% Saving

%save outputs_current_block_human index_human_block outputs_human_baseline outputs_human_block outputs_human_iso
